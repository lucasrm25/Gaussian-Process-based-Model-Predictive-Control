...
% load saved data
load(fullfile(pwd,'/simresults/20-01-17-out-inverted-pendulum-trueModel.mat'));
out_true = out;
load(fullfile(pwd,'/simresults/20-01-17-out-GP-inverted-pendulum.mat')); %,'d_GP'
out_nom = out;
clearvars d_GP
%% True Dynamics Model
%--------------------------------------------------------------------------
...
%% plot results for estModel = nomModel,  estModel = nomModel with GP, estModel = trueModel
 
figure; 
subplot(2,1,1); hold on; grid on;
plot(out.t(1:end-1), out.r(2,:),'DisplayName', 'r(t)')
plot(out.t, out.x(3,:), 'DisplayName', 'x(t) estimated dynamics [rad]')
plot(out_nom.t, out_nom.x(3,:), 'DisplayName', 'x(t) nominal dynamics [rad]')
plot(out_true.t, out_true.x(3,:), 'DisplayName', 'x(t) true dynamics [rad]')
ylabel('[rad]');
legend;
subplot(2,1,2); hold on; grid on;
plot(out.t(1:end-1), out.u, 'DisplayName', 'u(t)')

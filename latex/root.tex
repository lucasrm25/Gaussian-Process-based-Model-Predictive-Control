...
\begin{figure}
  ...
  \caption{A schematic drawing of the single-track model. The vehicle mass and the vehicle moment of inertia (yaw axis) are given by $M$ and $I$, respectively. The distance from the front wheel as well as from the rear wheel to the center of mass are defined by $L_f$ and $L_f$, respectively. $\psi$ is the yaw angle. The forces which act on the rear and front wheel in longitudinal and lateral direction are named $F_{r,long}$, $F_{r,lat}$, $F_{f,long}$ and $F_{f,lat}$. The rear-slip angle is $\alpha_r$ and the front-slip angle is $\alpha_f$ }

...
%------------------------------------------------------------------
    ...
    methods
        ...
        %------------------------------------------------------------------
            ...
            [~, ia, ic]=unique(round(obj.track_c,4)','stable','rows');
            obj.track_c=obj.track_c(:,ia);
            obj.track_r=obj.track_r(:,ia);
            obj.track_l=obj.track_l(:,ia);
            obj.psi_c = obj.psi_c(ia);
            
            % parametrize track by traveled distance
            distFromCenter = sqrt(obj.track_c(1,:).^2 + obj.track_c(2,:).^2);
            obj.dist = cumsum(abs(conv(distFromCenter,[1+1e-15,-1],'valid')) );
            
            dist=zeros(length(distFromCenter),1);
            for i=2:length(distFromCenter)
                dist(i)=dist(i-1)+sqrt((obj.track_c(1,i)-obj.track_c(1,i-1)).^2 + (obj.track_c(2,i)-obj.track_c(2,i-1)).^2 );
            end
            obj.dist=dist';
            obj.w = w;
        end
        ...
        %------------------------------------------------------------------    
            ...
            offroad_error = (abs(countour_error) > R_c) * (100+100*(abs(countour_error) - R_c));
        end
